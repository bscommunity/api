...
private suspend fun createSeedThemes(
    ...
    data class Seed(val name: String, val replaces: String)
    val seeds = listOf(
        Seed("Midnight Dark", "ultraviolet"),
        Seed("Ocean Blue", "poseidon"),
        Seed("Sunset Glow", "summer-sundown"),
        Seed("Forest Green", "green-game-jam-2024"),
        Seed("Neon Pulse", "neon"),
        Seed("Pastel Dreams", "cherry-blossom"),
    )
    seeds.forEachIndexed { i, s ->
        try {
            val owner = userIds[i % userIds.size]
            val theme = repo.createTheme(userId = owner, name = s.name, replaces = s.replaces, id = null, originalArtwork = null)
