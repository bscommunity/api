 ...
 * `isExplicit`/`previewVideoId`) on every website publish.
 *
 * This DTO only models the fields the route actually consumes (the track audio
 * preview is always derived server-side from storage, never overridden).
 * Everything else the website sends is ignored via `ignoreUnknownKeys`, so this
 * decode effectively never fails on a well-formed website payload.
 *
